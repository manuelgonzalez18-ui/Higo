-- Rebind the service endpoint name to the suspension-safe implementation.
-- Existing PHP clients may continue calling register_membership_payment_v2;
-- internally it now routes through v3, which preserves disciplinary blocks.
...
begin
    if to_regprocedure(
        'public.register_membership_payment_core(uuid,uuid,text,text,text,text,numeric,numeric,date,text,jsonb)'
    ) is null then
        alter function public.register_membership_payment_v2(
            uuid, uuid, text, text, text, text, numeric, numeric, date, text, jsonb
        ) rename to register_membership_payment_core;
    end if;
end $$;
...
revoke all on function public.register_membership_payment_core(
    uuid, uuid, text, text, text, text, numeric, numeric, date, text, jsonb
...
grant execute on function public.register_membership_payment_core(
    uuid, uuid, text, text, text, text, numeric, numeric, date, text, jsonb
) to service_role;

-- Fix admin_get_ride_detail for production support_threads schemas that do not
-- expose an updated_at column. Ordering is derived from the JSON row so absent
-- timestamp keys safely resolve to NULL instead of failing function execution.

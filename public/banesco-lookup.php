...
function bl_find_config_path(): ?string {
    ...
    // En Hostinger el archivo vive en /home/<user>/public_html/banesco-lookup.php
    // → dirname(__DIR__) = /home/<user>
...
if ($configPath === null) {
    bl_fail(503, "Config no encontrado. Debe existir /home/<user>/private/higo-banesco.php");
}
$cfg = require $configPath;
if (!is_array($cfg)) {
    bl_fail(503, "Config inválido: el require no retornó array.");
}
...
function bl_log_request(string $path, array $payload): void {
    $json = (string) json_encode($payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_PRESERVE_ZERO_FRACTION);
...
function bl_log_response(string $path, int $httpCode, $body): void {
    ...
    $json = (string) json_encode($dump, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_PRESERVE_ZERO_FRACTION);
...
/**
 * Devuelve [payload enviado, http code, body crudo de Banesco].
 *

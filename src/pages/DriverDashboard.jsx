...
const DriverDashboard = () => {
    ...
    useEffect(() => {
        ...
        if (activeRide) {
            // Also clear pending requests when starting a ride
            setRequests([]);
    ...
    const processRequests = useCallback((incomingRides, replace = false) => {
        if (!profile || activeRideRef.current) return;
        ...
        const startTracking = async () => {
            ...
            watcherId = await BackgroundGeolocation.addWatcher(
                ...
                async (location, error) => {
                    ...
                    // Thresholds: Speed > 1.5m/s (~5.4km/h) OR Distance > 15m
                    if (speed > 1.5 && gpsBearing) {
                        ...
                        // Only visual rotate if moved > 8 meters (Refined for sharper response)
                        if (dist > 0.008) {
                        ...
                        activeRide
                            ? (navStep === 1
                                ...
                                : (navStep === 2
                                    ...
                                    : null))
                            : (isOnline && (lastLocationRef.current || currentLoc)
                                ? { lat: lastLocationRef.current?.latitude || currentLoc?.lat, lng: lastLocationRef.current?.longitude || currentLoc?.lng }
                                : null)
                    }
                    destination={
